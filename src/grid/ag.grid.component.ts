...
export class AgGridNg2Component extends ComponentBase {
    // HostBinding host block
    @HostBinding('style.display') display = 'block';

    // HostBinding host class
    @HostBinding('class') class = 'ag-dark';
    ...
    @ContentChildren(AgGridColumnComponent) public columns: QueryList<AgGridColumnComponent>;

    constructor(elementDef: ElementRef,
        private viewContainerRef: ViewContainerRef,
        private ng2FrameworkFactory: AgFrameworkFactory,
    ...
    @Input() columnSelector: boolean = false;
    ...
    ngAfterViewInit(): void {
        ...
        if (this.columns && this.columns.length > 0) {
            this.gridOptions.columnDefs = this.columns
                .map((column: AgGridColumnComponent) => {
    ...
    headerCellRendererFunc(params) {
        let eHeader = $('<span>');
        eHeader.text(`${params.colDef.headerName}`);
        $(params.eHeaderCell).click(() => {
            params.api.clearRangeSelection();

            params.api.addRangeSelection({
                rowStart: 0,
                rowEnd: params.api.getModel().rowsToDisplay.length - 1,
                columnStart: params.colDef.field,
                columnEnd: params.colDef.field
            });
        });
